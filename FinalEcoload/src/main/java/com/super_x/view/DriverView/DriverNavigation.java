...
public final class DriverNavigation {
    ...
    public static VBox createSidebar(String activePage) {
        ...
        logout.setOnAction(event -> HomePage.homeStage.setScene(new Login().getScene()));
    ...
    public static HBox createNavbar() {
        ...
        Label welcome = new Label("Welcome Back, Kishor Kalane");
        ...
        vehicle.getChildren().addAll(number, vehicleName);
    ...
    private static void navigate(String page) {
        Scene scene = switch (page) {
            ...
            case "Support" -> new Support().getSupportPageScene();
            default -> null;
        };
        if (scene != null) {
            HomePage.homeStage.setScene(scene);
