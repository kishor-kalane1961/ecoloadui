...
public class Available_Loads {
    ...
    private HBox loadCard(RowData row) {
        VBox identity = new VBox(8, label(row.company, 20, "#111111", true), status(row.status));
    ...
    private VBox match(int value) {
        Circle base = new Circle(36, Color.TRANSPARENT);
        base.setStroke(Color.web("#E0E3DF"));
        base.setStrokeWidth(5);
        Arc arc = new Arc(0, 0, 36, 36, 90, -value * 3.6);
        arc.setType(ArcType.OPEN);
        arc.setFill(Color.TRANSPARENT);
        arc.setStroke(Color.web(GREEN));
        arc.setStrokeWidth(5);
        VBox words = new VBox(1, label(value + "%", 14, GREEN, true), label("Match", 10, DARK, false));
        words.setAlignment(Pos.CENTER);
        StackPane ring = new StackPane(base, arc, words);
        VBox wrap = new VBox(ring);
    ...
    private void applyFilters() {
        ...
        List<RowData> result = allLoads.stream()
                .filter(x -> from.isEmpty() || x.source.toLowerCase(Locale.ROOT).contains(from))
                .filter(x -> to.isEmpty() || x.destination.toLowerCase(Locale.ROOT).contains(to))
    ...
    private record RowData(String company, String status, String source, String destination, String distance,
            String time, String cargo, String weight, int offer, int rate, int match) {
    }
}